(function () {
  ...
  function burst(o) {
    ...
    const colors = o?.colors || ["#ffd6a5", "#caffbf", "#bdb2ff", "#a0c4ff", "#ffadad"], labels = o?.labels || [], durationMs = o?.durationMs ?? 2200, count = o?.count ?? 280, imgSize = (o?.imgSize ?? 22) * DPR;
    ...
    function tick(now) {
      ...
      const t = (now - start) / durationMs; const fade = t > 0.7 ? Math.max(0, 1 - (t - 0.7) / 0.3) : 1;
      ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.clearRect(0, 0, W, H); ctx.globalAlpha = fade;
  ...
  function startCustomConfetti(o) { burst(o || {}) }
  window.startThemedConfetti = startThemedConfetti; window.startEmojiConfetti = startEmojiConfetti; window.startNormalConfetti = startNormalConfetti; window.startCustomConfetti = startCustomConfetti; window.prepareConfetti = () => ensureCanvas();
})();
