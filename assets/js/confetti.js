(function () {
  ...
  function resize(force) {
    if (!cvs) return;
    const w = Math.max(1, (innerWidth | 0) * DPR), h = Math.max(1, (innerHeight | 0) * DPR);
  ...
  function wait(keys) {
    return new Promise(r => {
      ...
      if (ok()) return r();
      const i = setInterval(() => { if (ok()) { clearInterval(i); r(); } }, 30);
    });
  }
  ...
  function burst(o) {
    ...
    function draw(p) {
      if (p.label && IMAGES[p.label] && LOADED[p.label]) {
        const s = p.size; ctx.drawImage(IMAGES[p.label], -s / 2, -s / 2, s, s);
      } else {
        ctx.fillStyle = p.color; ctx.beginPath();
        ctx.moveTo(-5 * DPR, -3 * DPR); ctx.lineTo(5 * DPR, -3 * DPR);
        ctx.lineTo(5 * DPR, 3 * DPR); ctx.lineTo(-5 * DPR, 3 * DPR);
