(function () {
  ...
  function ensureCanvas() {
    ...
    if (!cvs) { cvs = document.createElement("canvas"); cvs.id = "kc-confetti"; Object.assign(cvs.style, { position: "fixed", inset: "0", pointerEvents: "none", zIndex: "99999" }); document.body.appendChild(cvs) }
    ctx = cvs.getContext("2d"); resize(true); window.addEventListener("resize", () => resize(false), { passive: true })
  }
  ...
  function burst(o) {
    ensureCanvas(); resize(false); if (!ctx) return;
    ...
    const parts = Array.from({ length: count }, () => ({
      ...
      vx: (Math.random() * 2 - 1) * .7 * DPR, vy: (1.3 + Math.random() * 1.5) * DPR,
      rot: Math.random() * Math.PI, vr: (Math.random() * .22 - .11),
      color: colors[(Math.random() * colors.length) | 0],
      label: labels.length ? labels[(Math.random() * labels.length) | 0] : null,
      size: imgSize * (.85 + Math.random() * .5)
    }));

    function draw(p) {
      if (p.label && IMAGES[p.label] && LOADED[p.label]) { const s = p.size; ctx.drawImage(IMAGES[p.label], -s / 2, -s / 2, s, s) }
      else { ctx.beginPath(); ctx.moveTo(-5 * DPR, -3 * DPR); ctx.lineTo(5 * DPR, -3 * DPR); ctx.lineTo(5 * DPR, 3 * DPR); ctx.lineTo(-5 * DPR, 3 * DPR); ctx.closePath(); ctx.fillStyle = p.color; ctx.fill() }
    }
