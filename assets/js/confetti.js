(function () {
  ...
  function burst(o) {
    ...
    function tick(now) { const dt = Math.min((now - last) / 16.666, 3); last = now; ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.clearRect(0, 0, W, H); for (const p of parts) { p.vy += .007 * DPR * dt; p.vx *= .998; p.vy *= .998; p.x += p.vx * dt; p.y += p.vy * dt; p.rot += p.vr * dt } for (const p of parts) { ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.rot); draw(p); ctx.restore() } if (now - start < durationMs) rafId = requestAnimationFrame(tick); else { ctx.clearRect(0, 0, W, H); rafId = null } }
