(function () {
  ...
  const IMAGES = {}, LOADED = {};
  for (const k in IMG_SRC) { const img = new Image(); img.onload = () => LOADED[k] = true; img.onerror = () => LOADED[k] = false; img.src = IMG_SRC[k]; IMAGES[k] = img; LOADED[k] = false }
  ...
  function burst(o) {
    ...
    const colors = o?.colors || ["#ffd6a5", "#caffbf", "#bdb2ff", "#a0c4ff", "#ffadad"], labels = o?.labels || [], durationMs = o?.durationMs ?? 2400, count = o?.count ?? 280, imgSize = (o?.imgSize ?? 22) * DPR;
    ...
    function tick(now) {
      ...
      const t = (now - start) / durationMs; const fade = t > 0.8 ? Math.max(0, 1 - (t - 0.8) / 0.2) : 1;
      ...
      if (now - start < durationMs) { rafId = requestAnimationFrame(tick) } else { ctx.globalAlpha = 1; ctx.clearRect(0, 0, W, H); rafId = null }
    }
  ...
  function startEmojiConfetti() { burst({ colors: ["#e0ffe5", "#e8f0ff", "#ffe8f6"], labels: [], durationMs: 2000, count: 240 }) }
